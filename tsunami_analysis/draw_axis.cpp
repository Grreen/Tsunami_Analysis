...
void draw_grid(const coord_gl& pos_z, const coord_gl& pos_z1, const coord_gl& pos_z2, const coord_gl& pos_z3){
	...
	for (int i = 0; i < 3; i++){
		...
		glVertex3d(a_z[i].x, z * s_param.zoom + 5 * s_param.zoom, a_z[i].y);
	}
	...
	for (double i = int(gist_d.start_x) - 1; i < int(gist_d.end_x) + 1; i += gl_data.step_axis.x){
		if (i >= gist_d.start_x && i <= gist_d.end_x) {
			const coord_gl a = get_coord_gl(GLdouble(i), GLdouble(gist_d.start_y));
			const coord_gl b = get_coord_gl(GLdouble(i), GLdouble(gist_d.end_y));
	...
	for (double i = int(gist_d.start_x) - 1; i < int(gist_d.end_x) + 1; i += gl_data.step_font.x){
		if (i >= gist_d.start_x && i <= gist_d.end_x){
			double delta = 0;
			const coord_gl a = get_coord_gl(GLdouble(i), GLdouble(pos_z3.y));
			//b = get_coord_gl(GLdouble(i), GLdouble(gist_d.end_y));
			glBegin(GL_POLYGON);

			glVertex3d(a.x - 0.6 - delta / 4, 0.01, a.y - 0.6);
			glVertex3d(a.x - 0.6 - delta / 4, 0.01, a.y + 0.6);
			glVertex3d(a.x + 0.6 + delta / 4, 0.01, a.y + 0.6);
			glVertex3d(a.x + 0.6 + delta / 4, 0.01, a.y - 0.6);
			//////////////////////////////////////////////////////
			glVertex3d(a.x - 0.6 - delta / 4, 0.01, a.y + 0.6);
			glVertex3d(a.x - 0.6 - delta / 4, -1.0, a.y + 0.6);
			glVertex3d(a.x + 0.6 + delta / 4, -1.0, a.y + 0.6);
			glVertex3d(a.x + 0.6 + delta / 4, 0.01, a.y + 0.6);
	...
	for (double i = int(gist_d.start_y) - 1; i < int(gist_d.end_y) + 1; i += gl_data.step_font.y){
		if (i >= gist_d.start_y && i <= gist_d.end_y) {
			double delta = 0;
			const coord_gl a = get_coord_gl(GLdouble(pos_z3.x), GLdouble(i));
...
void draw_string(const point_gl& c_text, GLdouble scaled, GLdouble angle[3], const std::string& str) {
...
void draw_font_axis(const coord_gl& z, double x, double y, double step[2], double angle[3], double d_x[2], double d_y[2], double d_z[2]){
	...
	GLdouble max_h = get_max_all_h();
		...
		for (int i = int(gl_data.step_font.z); i < int(max_h) + 4; i += int(gl_data.step_font.z)){
			draw_string({ z.x + dx, i * s_param.zoom + 20 * gl_data.axis_font_size, z.y + dy }, gl_data.axis_font_size, angle, to_string(i));
			}
}
//void draw_font_y(){}
//void draw_font_z(){}
...
void draw_font(){
	...
	if (gl_data.looking_at == L"NORTH"){ 
		...
		draw_string({ get_coord_gl(x, y).x + dx[1]/1.5, dz[0], get_coord_gl(x, y).y + 2 * dy[0] }, gl_data.axis_font_size, angle, "E");
	...
	if (gl_data.looking_at == L"WEST"){ 
		...
		draw_string({ get_coord_gl( x - (x - gist_d.start_x)/2, y).x + dx[1], dz[0], get_coord_gl(x, y).y + dy[0]*1.5 }, gl_data.axis_font_size, angle, "E");
	...
	if (gl_data.looking_at == L"EAST"){ 
		...
		draw_string({ get_coord_gl(x + (gist_d.end_x - x) / 2, y).x + dx[1], dz[0], get_coord_gl(x, y).y + dy[0]*1.5}, gl_data.axis_font_size, angle, "E");
		draw_string({ get_coord_gl(x, y).x + 2 * dx[1], dz[1], get_coord_gl(x, y).y + dy[0] / 1.5 }, gl_data.axis_font_size, angle, "N");
	}
