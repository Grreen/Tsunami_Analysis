...
namespace nana
{
	...
	public:
		...
		{
			typedef typename nana::detail::type_escape<Key>::type key_t;
			std::unique_ptr<nana::detail::key_interface> p(std::make_unique <nana::key<key_t, std::less<key_t> >>(kv));
			_m_erase(p.get());
		}
		...
		{
			typedef typename nana::detail::type_escape<Key>::type key_t;
			std::unique_ptr<nana::detail::key_interface> p(std::make_unique <nana::key<key_t, std::less<key_t> >>(std::move(kv)));
			_m_erase(p.get());
		}
