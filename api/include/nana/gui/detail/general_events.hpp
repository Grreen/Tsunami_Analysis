...
namespace nana
{
	namespace detail
	{
		...
		void __cdecl events_operation_register(event_handle);
		void __cdecl events_operation_cancel(event_handle);
	...
	public:
		...
		event_handle connect_front(Function && fn)
		{
			...
			std::unique_ptr<docker> dck(std::make_unique<docker>(this, factory<prototype, std::is_bind_expression<prototype>::value>::build(std::forward<Function>(fn)), false));
		...
		event_handle connect_unignorable(Function && fn, bool in_front = false)
		{
			...
			using prototype = typename std::remove_reference<Function>::type;
			std::unique_ptr<docker> dck(std::make_unique<docker>(this, factory<prototype, std::is_bind_expression<prototype>::value>::build(std::forward<Function>(fn)), true));
			auto evt = reinterpret_cast<event_handle>(static_cast<detail::docker_interface*>(dck.get()));
			if (in_front)
				dockers_->emplace(dockers_->begin(), std::move(dck));
			else
